        ...
        <h1>Workflow Error Dashboard</h1>
         <Panel header="Filter Workflows Errors (Press Enter to search)">
          ...
          {unknownErrors !== undefined && unknownErrors.map(item=>(
                                        <div>
                                         <Link to={'/workflow/errorDashboard/details/'+item.id+'/'+this.state.search+'/'+this.state.datefrm+'/'+this.state.dateto+'/'+this.state.range }>Error Details</Link>
           ...
           {knownErrors !== undefined && knownErrors.map(item=>(
                  <div>
                  <Link to={'/workflow/errorDashboard/details/'+item.id+'/'+this.state.search+'/'+this.state.datefrm+'/'+this.state.dateto+'/'+this.state.range }>{item.lookup}</Link>
                   <label className="small nobold">Total Count:{item.totalCount} </label><br/>
                   </div>
                ))}
            </Panel>
