...
public class WorkflowResource {
    ...
    @ApiOperation("Clones a completed workflow")
    @ApiResponses(value = {
            @ApiResponse(code = 404, message = "NOT_FOUND", response = Error.class),
            @ApiResponse(code = 400, message = "INVALID_INPUT", response = Error.class),
            @ApiResponse(code = 409, message = "CONFLICT", response = Error.class),
    ...
    private Response startWorkflow(HttpHeaders headers, Workflow workflow, String correlationId) throws Exception {
        StartWorkflowRequest request = new StartWorkflowRequest();
        request.setName(workflow.getWorkflowType());
        request.setVersion(workflow.getVersion());
        request.setCorrelationId(correlationId);
        request.setJobPriority(workflow.getJobPriority());
