...
BEGIN
    SELECT CASE WHEN 
        (SELECT COUNT(INDEX_NAME) FROM all_indexes WHERE OWNER = USER AND TABLE_NAME = 'WORKFLOW_PENDING' AND INDEX_NAME = 'UNIQUE_WORKFLOW_TYPE_WORKFLOW_ID') > 0 
        THEN
            'ALTER TABLE WORKFLOW_PENDING DROP CONSTRAINT UNIQUE_WORKFLOW_TYPE_WORKFLOW_ID DROP INDEX'
        ELSE
            'SELECT 1 FROM DUAL'
        END
    INTO sql_query from DUAL;
