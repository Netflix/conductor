...
public class OracleExecutionDAO extends OracleBaseDAO implements ExecutionDAO, RateLimitingDAO, PollDataDAO {
    ...
    private void addWorkflow(Connection connection, Workflow workflow) {
        String INSERT_WORKFLOW = "INSERT INTO workflow (workflow_id, correlation_id, json_data) VALUES (?, ?, utl_raw.cast_to_raw(?))";
    ...
    private void updateWorkflow(Connection connection, Workflow workflow) {
        String UPDATE_WORKFLOW = "UPDATE workflow SET json_data = utl_raw.cast_to_raw(?), modified_on = CURRENT_TIMESTAMP WHERE workflow_id = ?";
    ...
    private void insertOrUpdateTaskData(Connection connection, Task task) {
         ...
         */
        String UPDATE_TASK = "UPDATE task SET json_data=utl_raw.cast_to_raw(?), modified_on=CURRENT_TIMESTAMP WHERE task_id=?";
        int rowsUpdated = query(connection, UPDATE_TASK,
            q -> q.addJsonParameter(task).addParameter(task.getTaskId()).executeUpdate());

        if (rowsUpdated == 0) {
            String INSERT_TASK = "INSERT INTO task(task_id, json_data, modified_on) VALUES (?, utl_raw.cast_to_raw(?), CURRENT_TIMESTAMP)";
    ...
    private void updateEventExecution(Connection connection, EventExecution eventExecution) {
        // @formatter:off
        String UPDATE_EVENT_EXECUTION = "UPDATE event_execution SET " + "json_data = utl_raw.cast_to_raw(?), "
    ...
    private void insertOrUpdatePollData(Connection connection, PollData pollData, String domain) {
         ...
         */
        String UPDATE_POLL_DATA = "UPDATE poll_data SET json_data=utl_raw.cast_to_raw(?), modified_on=CURRENT_TIMESTAMP WHERE queue_name=? AND domain=?";
        ...
        if (rowsUpdated == 0) {
            String INSERT_POLL_DATA = "INSERT INTO poll_data(queue_name, domain, json_data, modified_on) VALUES (?, ?, utl_raw.cast_to_raw(?), CURRENT_TIMESTAMP)";
