...
public class OracleMetadataDAO extends OracleBaseDAO implements MetadataDAO, EventHandlerDAO {
    ...
    public void updateEventHandler(EventHandler eventHandler) {
        ...
        final String UPDATE_EVENT_HANDLER_QUERY = "UPDATE meta_event_handler SET " +
            "event = ?, active = ?, json_data = utl_raw.cast_to_raw(?), " +
        ...
        if (!workflowExists(tx, def)) {
            ...
            final String UPDATE_WORKFLOW_DEF_QUERY =
                "UPDATE meta_workflow_def " +
                    "SET json_data = utl_raw.cast_to_raw(?), modified_on = CURRENT_TIMESTAMP " +
    ...
    private String insertOrUpdateTaskDef(TaskDef taskDef) {
        final String UPDATE_TASKDEF_QUERY = "UPDATE meta_task_def SET json_data = utl_raw.cast_to_raw(?), modified_on = CURRENT_TIMESTAMP WHERE name = ?";

        final String INSERT_TASKDEF_QUERY = "INSERT INTO meta_task_def (name, json_data) VALUES (?, utl_raw.cast_to_raw(?))";
