...
public class OracleMetadataDAO extends OracleBaseDAO implements MetadataDAO, EventHandlerDAO {
    ...
    public OracleMetadataDAO(ObjectMapper objectMapper, DataSource dataSource, OracleProperties properties) {
            ...
            return q.executeAndFetch(rs -> {
                ...
                while (rs.next()) {
                    try {
                		...
                		EventHandler h = readValue(ClobHandler.clobToString(clob), EventHandler.class);
    ...
    private Optional<Integer> getLatestVersion(Connection tx, String name) {
        final String GET_LATEST_WORKFLOW_DEF_VERSION = "SELECT max(version) FROM meta_workflow_def WHERE " +
            "name = ?";
    	...
    	try {
            ...
            final String INSERT_WORKFLOW_DEF_QUERY = "INSERT INTO meta_workflow_def (name, version, json_data) VALUES (?," +
                ...
                if (!workflowExists(tx, def)) {
                    ...
                    execute(tx, UPDATE_WORKFLOW_DEF_QUERY, q -> q.addParameter(json)
			...
			logger.error("Error while inserting or updating Workflow Def JSON DATA - {}", e.getMessage());
		}
        ...
        return getWithRetriedTransactions(tx -> {
        	try {
        		...
        		execute(tx, UPDATE_TASKDEF_QUERY, update -> {
                    int result = update.addParameter(json).addParameter(taskDef.getName()).executeUpdate();
                    if (result == 0) {
                        execute(tx, INSERT_TASKDEF_QUERY,
                            insert -> insert.addParameter(taskDef.getName()).addParameter(json).executeUpdate());
                    }
                });
        		...
        		logger.error("Error while inserting or updating Task Def JSON DATA - {} - JSON {}", e.getMessage(), taskDef.toString());
        		e.printStackTrace();
    			return null;
        	}
        });
