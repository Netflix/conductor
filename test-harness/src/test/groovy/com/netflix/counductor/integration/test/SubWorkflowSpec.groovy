...
class SubWorkflowSpec extends Specification{
    ...
    def "Test workflow with subworkflow completion"() {
        ...
        pollAndCompleteTask1Try1 = workflowTestUtil.pollAndCompleteTask('simple_task_in_sub_wf', 'task1.integration.worker', ['op': 'simple_task_in_sub_wf.done'])
        ...
        workflow.output.get('op') == 'simple_task_in_sub_wf.done'
    }
    ...
    def "Test workflow with subworkflow failure and retry"() {
        ...
        then: "verify that the 'simple_task_in_sub_wf' is failed"
        ...
        then: "verify that the workflow is still RUNNING and SUB_WORKFLOW task is retried"
        workflow.getStatus() == Workflow.WorkflowStatus.RUNNING
        workflow.tasks.size() == 3
        workflow.tasks[1].taskType == 'SUB_WORKFLOW'
        workflow.tasks[1].status == Task.Status.FAILED
        workflow.tasks[2].taskType == 'SUB_WORKFLOW'
        workflow.tasks[2].status == Task.Status.SCHEDULED

        when: "Polled for and executed subworkflow task"
        polledTaskIds = queueDAO.pop("SUB_WORKFLOW", 1, 200);
        workflowExecutor.executeSystemTask(systemTask, polledTaskIds.get(0), 30)
        workflow = workflowExecutionService.getExecutionStatus(workflowInstanceId, true)

        then: "verify that the 'sub_workflow_task' is IN_PROGRESS"
        workflow.getStatus() == Workflow.WorkflowStatus.RUNNING
        workflow.getTasks().size() == 3
        workflow.tasks[2].taskType == 'SUB_WORKFLOW'
        workflow.tasks[2].status == Task.Status.IN_PROGRESS

        when: "Checking Subworkflow created by above sub_workflow_task"
        subWorkflowId = workflow.tasks[2].subWorkflowId
        subWorkflow = workflowExecutionService.getExecutionStatus(subWorkflowId, true)
        ...
        then: "verify that the workflow is FAILED"
        workflow.getStatus() == Workflow.WorkflowStatus.FAILED
        workflow.tasks.size() == 3
        workflow.tasks[1].taskType == 'SUB_WORKFLOW'
        workflow.tasks[1].status == Task.Status.FAILED
        workflow.tasks[2].taskType == 'SUB_WORKFLOW'
        workflow.tasks[2].status == Task.Status.FAILED
        ...
        workflow.getTasks()[1].status == Task.Status.FAILED
        workflow.getTasks()[2].taskType == 'SUB_WORKFLOW'
        workflow.getTasks()[2].status == Task.Status.IN_PROGRESS
    }
}
