...
public class TaskLogExport extends AbstractExport {
    private static final Logger logger = LogManager.getLogger(TaskLogExport.class);
    private static final SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMww");
    static {
        sdf.setTimeZone(TimeZone.getTimeZone("GMT"));
    }

    public TaskLogExport(RestHighLevelClient client, EntityWriters writers) {
    ...
    public void export() throws IOException {
        ...
        findAndExport(created, indexPrefix + ".event", "event");
        findAndExport(created, indexPrefix + ".message", "message");
        findAndExport(createdTime, indexPrefix + ".task", "task");

        logger.info("TaskLogExport done");
    }

    private void findAndExport(QueryBuilder query, String indexName, String typeName) throws IOException {
        logger.info("findAndExport started for " + indexName + "/" + typeName);
        AppConfig config = AppConfig.getInstance();
        ...
        try {
            while (searchHits != null && searchHits.length > 0) {
                for (SearchHit hit : searchHits) {
                    writers.TASK_LOGS(convert(wrap(hit)));
                }
    ...
    public void deleteEmptyTaskLogsIndexes() throws IOException {
