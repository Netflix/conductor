...
class MySQLMetadataDAO extends MySQLBaseDAO implements MetadataDAO {
	...
	private static final String INSERT_TASKDEF_QUERY = "INSERT INTO meta_task_def (name, json_data) VALUES (:name, :jsonData)";
	private static final String EXISTS_TASKDEF_QUERY = "SELECT EXISTS(SELECT 1 FROM meta_task_def WHERE name = :name)";
	private static final String UPDATE_TASKDEF_QUERY = "UPDATE meta_task_def SET json_data = :jsonData, modified_on = CURRENT_TIMESTAMP WHERE name = :name";
	private static final String READ_ONE_TASKDEF_QUERY = "SELECT json_data FROM meta_task_def WHERE name = :name";
	private static final String READ_ALL_TASKDEF_QUERY = "SELECT json_data FROM meta_task_def";
	private static final String DELETE_TASKDEF_QUERY = "DELETE FROM meta_task_def WHERE name = :name";

	private static final String INSERT_WORKFLOW_DEF_QUERY = "INSERT INTO meta_workflow_def (name, version, json_data) VALUES (:name, :version, :jsonData)";
	private static final String UPDATE_WORKFLOW_DEF_QUERY = "UPDATE meta_workflow_def SET json_data = :jsonData, modified_on = CURRENT_TIMESTAMP WHERE name = :name AND version = :version";
	private static final String GET_LATEST_WORKFLOW_DEF_VERSION = "SELECT max(version) AS version FROM meta_workflow_def WHERE name = :name";
	private static final String UPDATE_WORKFLOW_DEF_LATEST_VERSION_QUERY = "UPDATE meta_workflow_def SET latest_version = :latest_version WHERE name = :name";
	private static final String CHECK_WORKFLOW_DEF_EXISTS_QUERY = "SELECT COUNT(*) FROM meta_workflow_def WHERE name = :name AND version = :version";
	private static final String GET_LATEST_WORKFLOW_DEF_QUERY = "SELECT json_data FROM meta_workflow_def WHERE NAME = :name AND version = latest_version";
	private static final String GET_WORKFLOW_DEF_QUERY = "SELECT json_data FROM meta_workflow_def WHERE NAME = :name AND version = :version";
	private static final String FIND_ALL_WORKFLOW_DEF_QUERY = "SELECT DISTINCT name FROM meta_workflow_def";
	private static final String GET_ALL_LATEST_WORKFLOW_DEF_QUERY = "SELECT json_data FROM meta_workflow_def WHERE version = latest_version";
	private static final String GET_ALL_VERSIONS_WORKFLOW_DEF_QUERY = "SELECT json_data FROM meta_workflow_def WHERE name = :name ORDER BY version";
	private static final String GET_ALL_WORKFLOW_DEF_QUERY = "SELECT json_data FROM meta_workflow_def ORDER BY name, version";

	private static final String INSERT_EVENT_HANDLER_QUERY = "INSERT INTO meta_event_handler (name, event, active, json_data) VALUES (:name, :event, :active, :jsonData)";
	private static final String UPDATE_EVENT_HANDLER_QUERY = "UPDATE meta_event_handler SET event = :event, active = :active, json_data = :jsonData, modified_on = CURRENT_TIMESTAMP WHERE name = :name";
	private static final String READ_ONE_EVENT_HANDLER_QUERY = "SELECT json_data FROM meta_event_handler WHERE name = :name";
	private static final String READ_ALL_EVENT_HANDLER_QUERY = "SELECT json_data FROM meta_event_handler";
	private static final String READ_ALL_EVENT_HANDLER_BY_EVENT_QUERY = "SELECT json_data FROM meta_event_handler WHERE event = :event";
	private static final String DELETE_EVENT_HANDLER_QUERY = "DELETE FROM meta_event_handler WHERE name = :name";
	...
	private String insertOrUpdateTaskDef(TaskDef taskDef) {
		withTransaction(connection -> {
			boolean exists = connection.createQuery(EXISTS_TASKDEF_QUERY).bind(taskDef).executeScalar(Boolean.class);
			if (!exists) {
				connection.createQuery(INSERT_TASKDEF_QUERY).bind(taskDef).addParameter("jsonData", toJson(taskDef)).executeUpdate();
			} else {
				connection.createQuery(UPDATE_TASKDEF_QUERY).bind(taskDef).addParameter("jsonData", toJson(taskDef)).executeUpdate();
			}
			refreshTaskDefs(connection);
		});
