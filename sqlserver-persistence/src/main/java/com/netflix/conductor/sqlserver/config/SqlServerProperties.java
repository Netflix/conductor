...
@ConfigurationProperties("conductor.sqlserver")
public class SqlServerProperties {

    /**
     * The jdbc url to be used for connecting to the database
     */
    private String jdbcUrl = "jdbc:sqlserver://sqlserver:1433;database=Conductor;encrypt=false;trustServerCertificate=true;";
    ...
    /**
     * Shared - all the conductor instances will share the same queues. decider service lock
     *          must be enabled
     * Local Only - Queues are per zone. Workflows started in zone A will only run in zone A.
     *              Does not require decider service lock.
     */
    public enum QUEUE_STRATEGY {SHARED, LOCAL_ONLY}

    /**
     * The queue sharding strategy, see sqlserver's README.md
     */
    private String queueShardingStrategy = QUEUE_STRATEGY.SHARED.toString();

    /**
     * The zone name. Limited to 4 characters. Default is GLBL (Global)
     */
    private String zone = "GLBL";

    private String lockNamespace = "";

    @DurationUnit(ChronoUnit.SECONDS)
    private Duration processAllRemovesInterval = Duration.ofSeconds(0);
    ...
    public QUEUE_STRATEGY getQueueShardingStrategyAsEnum() {
        return QUEUE_STRATEGY.valueOf(queueShardingStrategy);
    }
    ...
    public Duration getProcessAllRemovesInterval() {
        return processAllRemovesInterval;
    }

    public void setProcessAllRemovesInterval(Duration processAllRemovesInterval) {
        this.processAllRemovesInterval = processAllRemovesInterval;
    }
}
