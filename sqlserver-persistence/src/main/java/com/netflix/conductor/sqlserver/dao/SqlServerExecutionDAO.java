...
public class SqlServerExecutionDAO extends SqlServerBaseDAO implements ExecutionDAO, RateLimitingDAO, PollDataDAO {
    ...
    public SqlServerExecutionDAO(ObjectMapper objectMapper, DataSource dataSource) {
    ...
    /**
     * This is a dummy implementation and this feature is not supported for SqlServer backed Conductor
     */
    @Override
    public boolean removeWorkflowWithExpiry(String workflowId, int ttlSeconds) {
        throw new UnsupportedOperationException("This method is not implemented in SqlServerExecutionDAO. Please use RedisDAO mode instead for using TTLs.");
    }
    ...
    boolean addScheduledTask(Connection connection, Task task, String taskKey) {
        ...
        int count = query(connection, 
            ...
            q -> q.addParameter(task.getWorkflowInstanceId()).addParameter(taskKey).addParameter(task.getTaskId()).executeUpdate()
        );
        return count > 0;
    }
