...
class GenericHttpTask extends WorkflowSystemTask {
    ...
    public static final String CUSTOM_FAILURE_REASON_PARAMETER_NAME = "failure_reason";
    ...
    void setReasonForIncompletion(HttpResponse response, Task task) {
        Object failureReasonParam = task.getInputData().get(CUSTOM_FAILURE_REASON_PARAMETER_NAME);
        if (isNull(failureReasonParam)) {
            setHttpReasonForIncompletion(response, task);
        } else {
            checkCustomFailureReasonError(task, response);
        }
    }

    private void setHttpReasonForIncompletion(HttpResponse response, Task task) {
     ...
     */
    private void checkCustomFailureReasonError(Task task, HttpResponse response) {
        Object failureReasonParam = task.getInputData().get(CUSTOM_FAILURE_REASON_PARAMETER_NAME);
        if (isNull(failureReasonParam)) return;

        FailureReasonOutput output = om.convertValue(failureReasonParam, FailureReasonOutput.class);
        String failureReason = output.getReason();
        if (isNull(failureReason)) return;

        // Get the response map with details of the failure reason or skip if non-existent
        Map<String, Object> responseMap = response.asMap();
        if (isNull(responseMap) || responseMap.isEmpty()) return;

        // evaluate the provided script and set reason
        AtomicBoolean overallStatus = new AtomicBoolean(true);
        String reason = "";
        try {
            reason = (String) ScriptEvaluator.eval(failureReason, responseMap);
            addCustomFailureEvalResult(task, reason);
            overallStatus.set(false);
        } catch (Exception e) {
            logger.error(String.format("failed to evaluate failure reason for script = %s", failureReason), e);
        ...
        processCustomFailureReasonResponse(task, response, failureReason, overallStatus, reason);
    }

    private void processCustomFailureReasonResponse(Task task, HttpResponse response, String failureReason, AtomicBoolean overallStatus, String reason) {
        ...
        if (StringUtils.isEmpty(reason)) {
            ...
            genericTaskFailure(task, reason, overallStatus);
        }
    }
    ...
    protected void genericTaskFailure(Task task, String result, AtomicBoolean overallStatus) {
        if (!overallStatus.get()) {
            String overallReason = "Response validation failed";
            if (StringUtils.isNotEmpty(result)) {
                overallReason = result;
            }
