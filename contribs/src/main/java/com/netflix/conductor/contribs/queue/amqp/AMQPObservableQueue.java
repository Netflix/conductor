...
public class AMQPObservableQueue implements ObservableQueue {
	...
	private void publishMessage(Message message, String exchange, String routingKey) {
		try {
			...
			amqpConnection.getOrCreateChannel(ConnectionType.PUBLISHER, getSettings().getQueueOrExchangeName())
					.basicPublish(exchange, routingKey, buildBasicProperties(message, settings),
	...
	public long size() {
		try {
			return amqpConnection.getOrCreateChannel(ConnectionType.SUBSCRIBER, getSettings().getQueueOrExchangeName())
	...
	private AMQP.Exchange.DeclareOk getOrCreateExchange(ConnectionType connectionType, String name, final String type,
			final boolean isDurable, final boolean autoDelete, final Map<String, Object> arguments) throws IOException {
