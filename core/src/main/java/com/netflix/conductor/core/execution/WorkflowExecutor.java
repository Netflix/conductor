...
public class WorkflowExecutor {
	...
	public String startWorkflow(String workflowId, String name, int version, Map<String, Object> input,
		...
		// Decode auth context if passed and log username/email
		...
		if (authContextEnabled && StringUtils.isNotEmpty(contextToken)) {
			try {
				...
				contextInfo = String.format("%s(%s)", username, email);
			} catch (Exception ex) {
		...
		try {
			...
			logger.debug("Workflow has started. Current status=" + wf.getStatus() + ", workflowId=" + wf.getWorkflowId()
					+ ", correlationId=" + wf.getCorrelationId()+", input="+wf.getInput() + ", contextInfo=" + wf.getContextInfo());
			return workflowId;
	...
	private boolean rerunWF(String workflowId, String taskId, Map<String, Object> taskInput,
		...
		logger.debug("Workflow rerun.Current status=" + workflow.getStatus() + ",workflowId=" + workflow.getWorkflowId() + ",CorrelationId=" + workflow.getCorrelationId() + ",input=" + workflow.getInput() + ", contextInfo=" + workflow.getContextInfo());
		return false;
	}
	...
	public void rewind(String workflowId, String correlationId) throws Exception {
		...
		if (!workflow.getStatus().isTerminal()) {
			logger.error("Workflow is still running.  status=" + workflow.getStatus()+",workflowId="+workflow.getWorkflowId()+",correlationId="+workflow.getCorrelationId()+ ", contextInfo=" + workflow.getContextInfo());
		...
		decide(workflowId);
		logger.debug("Workflow rewind.Current status=" + workflow.getStatus() + ",workflowId=" + workflow.getWorkflowId()+",CorrelationId=" + workflow.getCorrelationId()+",input="+workflow.getInput() + ", contextInfo=" + workflow.getContextInfo());
	}
	...
	public void retry(String workflowId, String correlationId) throws Exception {
		...
		if (workflow.getTasks().isEmpty()) {
			logger.error("Workflow has not started yet.  status=" + workflow.getStatus()+",workflowId="+workflow.getWorkflowId()+",correlationId="+workflow.getCorrelationId() + ", contextInfo=" + workflow.getContextInfo());
		...
		decide(workflowId);
		logger.debug("Workflow retry.Current status=" + workflow.getStatus() + ",workflowId=" + workflow.getWorkflowId()+",CorrelationId=" + workflow.getCorrelationId()+",input="+workflow.getInput() + ", contextInfo=" + workflow.getContextInfo());
	}
	...
	public void completeWorkflow(Workflow wf) throws Exception {
		...
		if (workflow.getStatus().equals(WorkflowStatus.COMPLETED)) {
			logger.warn("Workflow has already been completed.  Current status=" + workflow.getStatus() + ", workflowId=" + wf.getWorkflowId()+",CorrelationId=" + wf.getCorrelationId() + ", contextInfo=" + workflow.getContextInfo());
			return;
		}

		if (workflow.getStatus().isTerminal()) {
			String msg = "Workflow has already been completed.  Current status " + workflow.getStatus();
			logger.error("Workflow has already been completed.  status=" + workflow.getStatus()+",workflowId="+workflow.getWorkflowId()+",correlationId="+workflow.getCorrelationId() + ", contextInfo=" + workflow.getContextInfo());
		...
		logger.debug("Workflow has completed, workflowId=" + wf.getWorkflowId()+",input="+wf.getInput()+",CorrelationId="+wf.getCorrelationId()+",output="+wf.getOutput() + ", contextInfo=" + workflow.getContextInfo());
	}
	...
	public String cancelWorkflow(Workflow workflow, String reason) throws Exception {
		...
		edao.updateWorkflow(workflow);
		queue.remove(deciderQueue, workflow.getWorkflowId());	//remove from the sweep queue
		logger.error("Workflow is cancelled.workflowId="+workflowId+",correlationId="+workflow.getCorrelationId() + ", contextInfo=" + workflow.getContextInfo());
		...
		logger.debug("Workflow has cancelled, workflowId=" + workflow.getWorkflowId()+",input="+workflow.getInput()+",CorrelationId="+workflow.getCorrelationId()+",output="+workflow.getOutput() + ", contextInfo=" + workflow.getContextInfo());
		return workflowId;
	}
	...
	public String reset(String workflowId, String reason) throws Exception {
		...
		edao.updateWorkflow(workflow);
		queue.remove(deciderQueue, workflow.getWorkflowId());	//remove from the sweep queue
		logger.error("Workflow has been reset.workflowId="+workflowId+",correlationId="+workflow.getCorrelationId() + ", contextInfo=" + workflow.getContextInfo());
		cancelTasks(workflow, workflow.getTasks());
	...
	public void terminateWorkflow(Workflow workflow, String reason, String failureWorkflow, Task failedTask) throws Exception {
		...
		edao.updateWorkflow(workflow);
		logger.error("Workflow is terminated/reset.workflowId="+workflowId+",correlationId="+workflow.getCorrelationId()+",reasonForIncompletion="+reason + ", contextInfo=" + workflow.getContextInfo());
		...
		if (!StringUtils.isBlank(failureWorkflow)) {
			...
			if (failedTask != null) {
				...
				logger.error("Error in task execution.workflowid="+workflowId+",correlationId="+workflow.getCorrelationId()+",failedTaskid="+failedTask.getTaskId()+",taskReferenceName="+failedTask.getReferenceTaskName()+"reasonForIncompletion="+failedTask.getReasonForIncompletion() + ", contextInfo=" + workflow.getContextInfo());
			}
	...
	public void updateTask(TaskResult result) throws Exception {
		...
		if (wf.getStatus().isTerminal()) {
			...
			String msg = "Workflow " + wf.getWorkflowId() + " is already completed as " + wf.getStatus() + ", task=" + task.getTaskType() + ", reason=" + wf.getReasonForIncompletion()+",correlationId="+wf.getCorrelationId() + ", contextInfo=" + wf.getContextInfo();
			logger.warn(msg);
		...
		if (task.getStatus().isTerminal()) {
			...
			queue.remove(QueueUtils.getQueueName(task), result.getTaskId());
			String msg = "Task is already completed as " + task.getStatus() + "@" + task.getEndTime() + ", workflow status=" + wf.getStatus() + ", workflowId=" + wf.getWorkflowId() + ", taskId=" + task.getTaskId()+",correlationId="+wf.getCorrelationId() + ", contextInfo=" + wf.getContextInfo();
			logger.warn(msg);
		...
		if (task2.getStatus().isTerminal()) {
			...
			String msg = "Task is already terminal as " + task2.getStatus() + "@" + task2.getEndTime() + ", workflow status=" + wf.getStatus() + ", workflowId=" + wf.getWorkflowId() + ", taskId=" + task2.getTaskId()+",correlationId="+wf.getCorrelationId() + ", contextInfo=" + wf.getContextInfo();
			logger.warn(msg);
			return;
		}
		edao.updateTask(task);
	...
	public boolean decide(String workflowId) throws Exception {
		...
		try {
			...
			String error = "Error in workflow execution: " + tw.getMessage() +
					", workflowId=" + workflow.getWorkflowId() + ", correlationId=" + workflow.getCorrelationId() + ", contextInfo=" + workflow.getContextInfo();
			if (tw.task != null) {
	...
	public void executeSystemTask(WorkflowSystemTask systemTask, String taskId, int callbackSeconds) {
		try {
			...
			logger.debug("Executing {}/{}-{} for workflowId={}, correlationId={}, contextInfo={}", task.getTaskType(), task.getTaskId(), task.getStatus(),
					workflow.getWorkflowId(), workflow.getCorrelationId(), workflow.getContextInfo());
			...
			logger.debug("Done Executing {}/{}-{} op={} for workflowId={}, correlationId={}, contextInfo={}",
	...
	private void terminate(final WorkflowDef def, final Workflow workflow, TerminateWorkflow tw) throws Exception {
		...
		logger.error("Workflow failed/reset. workflowId=" + workflow.getWorkflowId()+",correlationId="+workflow.getCorrelationId()+",Reason="+tw.getMessage()+",taskId="+taskId+",taskReferenceName="+taskRefName + ", contextInfo=" + workflow.getContextInfo());
	}
